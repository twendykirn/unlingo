...
export default function TranslationEditor() {
  ...
  const createNodesFromJson = useCallback((obj: any): TranslationNode[] => {
  ...
  const handleLanguageChange = (newLanguage: string) => {
    if (hasUnsavedChanges) {
      const confirmSwitch = window.confirm("You have unsaved changes. Switching language will discard them. Continue?");
  ...
  const handleVersionChange = (newVersion: string) => {
    if (hasUnsavedChanges) {
      const confirmSwitch = window.confirm("You have unsaved changes. Switching version will discard them. Continue?");
      if (!confirmSwitch) return;
  ...
  const deleteSelectedKey = () => {
    ...
    const buildJsonFromNodes = (nodeId: string): any => {
      ...
      if (node.type === 'object') {
        ...
        node.children.forEach(childId => {
          if (childId !== selectedNode) { // Skip deleted child
    ...
    rootNodes.forEach(rootNode => {
      if (rootNode.id !== selectedNode) { // Skip if root is being deleted
      ...
      return (
        <div key={node.id} className="select-none">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-gray-300 truncate ml-2 max-w-48">
                  {getDisplayValue()}
                </div>
              </div>
            </div>
          ...
          {hasChildren && isExpanded && (
            <div>
              {childNodes.map(childNode => renderTreeNode(childNode, level + 1))}
            </div>
          )}
        </div>
      );
                        ...
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            if (node.type === 'object' || node.type === 'array') {
                              ...
                              valueText = currentValue || JSON.stringify(node.value, null, 2);
                            } else {
                      ...
                      {node.type === 'string' && typeof node.value === 'object' && node.value ? (
                        <div className="space-y-4">
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <Button 
                                variant="outline" 
                                size="sm" 
                                onClick={() => copyToClipboard((node.value as any)[selectedLanguage] || '')}
                          ...
                          {Object.entries(node.value as object)
                            .filter(([lang]) => lang !== selectedLanguage && (node.value as any)[lang])
                            .length > 0 && (
                            <div>
                              ...
                              <div className="space-y-1 max-h-32 overflow-y-auto">
                                {Object.entries(node.value as object)
                                  .filter(([lang]) => lang !== selectedLanguage && (node.value as any)[lang])
                                  .map(([lang, text]) => (
                                  <div key={lang} className="text-xs text-gray-400 break-words">
                                    <span className="font-mono text-gray-500">{lang.toUpperCase()}:</span> {text as string}
                                  </div>
                                ))}
                              </div>
                            </div>
          ...
          <div className="bg-gray-900 rounded-lg p-6 w-4/5 h-4/5 max-w-6xl flex flex-col">
            ...
            <div className="flex-1 flex flex-col">
              <p className="text-sm text-gray-400 mb-4">
                Edit the entire JSON structure. Perfect for developers who want to copy/paste from existing codebases.
              </p>
          ...
          <div className="bg-gray-900 rounded-lg p-6 w-96 max-w-full">
            ...
            <div className="space-y-4">
              ...
              <div className="text-xs text-gray-500">
                <p><strong>Tip:</strong> Enter plain text for simple translations, or valid JSON for complex structures.</p>
              </div>
            </div>
